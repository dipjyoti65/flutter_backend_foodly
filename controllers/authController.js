...
module.exports = {
  ...
  loginUser:async(req,res)=>{
    ...
    try{
      ...
      const decrytedPassword = CryptoJS.decrypt(user.password,process.env.SECRET);

      const depassword = decrytedPassword.toString(CryptoJS.enc.Utf8);

      if(depassword !== req.body.passowrd){
      ...
      const{passowrd,otp,...others} = user.$getPopulatedDocs;
